const DesignStylesInput = ({ disabled, error, type }) => {
	...
	const styleType = [
		...
		'h-11 px-3 py-2 text-base md:text-sm shadow-xs'
	].find(x => x)

	const styleDisabled = disabled
		? ' text-disabled dark:text-disabledDark border-disabled dark:border-disabledDark'
		: ` text-primary dark:text-primaryDark placeholder-tertiary dark:placeholder-tertiaryDark border-line dark:border-lineDark focus:border-brand1 dark:focus:border-brand1Dark focus:ring-brand1 dark:focus:ring-brand1Dark`
